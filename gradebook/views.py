    ...
    if request.method == 'GET':
        if request.is_ajax():
            ...
            if get_action == 'get_category_id':
                ...
                if courses.__len__() == 1:
                    course = courses[0]
                    categories = Category.objects.filter(course=course, name=request.GET['category_name'])
                    if categories.__len__() >= 1:  # allows user to have more than one category of same name - bad
    ...
    if request.is_ajax():
        ...
        if post_action == 'add_category':
            ...
            if courses.__len__() == 1:
                ...
                course.not_specified_worth -= float(category_worth)
                category.save()
                course.save()
        ...
        if post_action == 'edit_category':
            ...
            if courses.__len__() == 1:
                ...
                if categories.__len__() == 1:
                    ...
                    category.worth = float(request.POST['category_worth'])
                    course.not_specified_worth -= category.worth
        ...
        if post_action == 'delete_category':
            ...
            if courses.__len__() == 1:
                ...
                if category.__len__() != 1:
                    return
                category = category[0]
                course.not_specified_worth += float(category.worth)
                category.delete()
                course.save()
        ...
        if post_action == 'add_assignment':
            courses = Course.objects.filter(id=course_id)
            categories = Category.objects.filter(id=request.POST['assignment_category_id'])
            if courses.__len__() == 1 and categories.__len__() == 1:
                course = courses[0]
                category = categories[0]
        ...
        if post_action == 'edit_assignment':
            courses = Course.objects.filter(id=course_id)
            assignments = Assignment.objects.filter(id=request.POST['assignment_id'])
            if courses.__len__() == 1 and assignments.__len__() == 1:
                course = courses[0]
